        ...
        if (request.ok) {
            request.json().then(function (response) {
                ...
                if (response.status) {
                    ...
                    document.getElementById('id_caso').value = response.dataset.id_caso;
                    document.getElementById('nombre_caso').value = response.dataset.nombre_caso;
                    document.getElementById('id_abogado').value = response.dataset.id_abogado;
                    document.getElementById('id_estado_caso').value = response.dataset.id_estado_caso;
                    document.getElementById('fecha_inicio').value = response.dataset.fecha_inicio;
                    document.getElementById('fecha_finalizacion').value = response.dataset.fecha_finalizacion;
                    document.getElementById('observaciones').value = response.dataset.observaciones;
                    document.getElementById('id_materia').value = response.dataset.id_materia;
                    fillSelect(ENDPOINT_CATEGORIAS, 'id_materia', response.dataset.id_materia);
                    if (response.dataset.id_materia) {
                        document.getElementById('id_materia').checked = true;
                    } else {
                        document.getElementById('id_materia').checked = false;
                    }
...
document.getElementById('save-form').addEventListener('submit', function (event) {
    ...
    (document.getElementById('id_caso').value) ? action = 'update' : action = 'create';
...
function openDeleteDialog(id) {
    // Se define un objeto con los datos del registro seleccionado.
    const data = new FormData();
    data.append('id_caso', id);
    // Se llama a la función que elimina un registro. Se encuentra en el archivo components.js
    confirmDelete(asignar_casos.php, data);
}